...
// ── Memory storage (works on Vercel serverless — no disk access) ──
const upload = multer({
...
router.post('/upload-pdf', upload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    // Extract text from buffer directly (no disk file)
    const { text, pages, info } = await extractTextFromPDF(req.file.buffer);
  ...
  try {
    ...
    history.forEach((item, i) => {
      ...
      if (item.sources && item.sources.length > 0) {
        ...
        item.sources.forEach((s, j) => { markdown += `> ${j + 1}. ${s.text}\n`; });
        markdown += '\n';
      }
      markdown += `---\n\n`;
    });
